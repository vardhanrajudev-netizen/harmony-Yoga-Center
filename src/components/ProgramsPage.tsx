...
export default function ProgramsPage({ onSelectProgram }: ProgramsPageProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute right-0 top-0 w-80 h-80 rounded-full bg-brand-sage/15 filter blur-3xl pointer-events-none ambient-glow-1" />
      ...
      {/* Hero Header Banner */}
      <section className="py-20 md:py-28 px-6 text-center select-none" id="programs-page-hero">
      ...
      <section className="py-8 px-6 sm:px-10 lg:px-16" id="comprehensive-program-tracks">
        <div className="max-w-[1280px] mx-auto">
          ...
          <div className="flex justify-center mb-16">
            <div className="inline-flex gap-2 p-2 rounded-full bg-white/90 border border-brand-sage/40 shadow-xs">
              {tracks.map((track) => (
                <button
                  key={track.id}
                  onClick={() => setActiveTab2(track.id as any)}
                  className={`px-5 sm:px-7 py-3 rounded-full text-[11px] sm:text-xs font-bold uppercase tracking-wider transition-all duration-300 cursor-pointer ${
                    activeTab === track.id 
                      ? 'bg-[#0F766E] text-brand-ivory shadow-md' 
                      : 'text-brand-charcoal/65 hover:text-brand-emerald hover:bg-brand-sage/20'
                  }`}
                >
                  {track.title}
                </button>
          ...
          {tracks.map((track) => {
            ...
            return (
              <motion.div
                ...
                className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center bg-white/95 border border-brand-sage/40 rounded-[32px] p-8 sm:p-14 shadow-[0_12px_40px_rgba(0,0,0,0.015)] relative"
              ...
              >
                ...
                <div className="lg:col-span-5 flex justify-center">
                  <div className="relative group w-full max-w-sm rounded-[24px] overflow-hidden shadow-2xl border border-brand-sage/30 aspect-[4/5] bg-brand-ivory">
                    <img 
                      src={track.image} 
                      alt={track.title}
                    ...
                    <div className="absolute bottom-4 left-4 right-4 bg-white/95 p-4 rounded-2xl border border-brand-sage/30 shadow-md">
                      <p className="text-[9px] font-bold text-[#b58552] tracking-wider uppercase font-sans">TARGET PHYSIOLOGY</p>
                      <p className="text-xs font-bold text-brand-emerald mt-1">{track.clinicalGoal}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="lg:col-span-7 space-y-6 font-sans">
                  <div>
                    ...
                    <h2 className="font-serif text-3xl sm:text-4xl font-bold text-brand-emerald tracking-tight mt-1 leading-tight">
                      {track.title}
                    </h2>
                    <p className="text-xs sm:text-sm text-brand-charcoal/70 leading-relaxed mt-4">
                      {track.description}
                    </p>
                  </div>

                  {/* Bullet Lists */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pb-2 border-y border-brand-sage/15 py-4">
                    {track.bulletPoints.map((bp, key) => (
                      <div key={key} className="flex items-start gap-2.5 text-xs text-brand-charcoal/80">
                  ...
                  {/* Action Area */}
                  <div className="pt-6 flex flex-col sm:flex-row gap-6 items-center justify-between" id={`action-area-${track.id}`}>
                    ...
                    <button
                      ...
                      className="group/btn h-[54px] w-full sm:w-auto px-8 rounded-full bg-[#0F766E] hover:bg-[#0D6962] text-brand-ivory text-xs font-bold uppercase tracking-widest inline-flex items-center justify-between gap-5 transition-all duration-300 shadow-[0_4px_12px_rgba(15,118,110,0.15)] hover:shadow-[0_8px_24px_rgba(15,118,110,0.25)] hover:-translate-y-0.5 cursor-pointer shrink-0"
                    >
                      <span>{track.id === 'personalized' ? 'Book Your Session' : 'Book Free Trial'}</span>
                      <div 
                        className="h-8.5 w-8.5 rounded-full bg-[#D4A373] group-hover/btn:bg-[#E2B383] flex items-center justify-center text-white transition-all duration-300 transform group-hover/btn:translate-x-[4px] shrink-0"
                        style={{ boxShadow: '0 0 15px rgba(212,163,115,0.4)' }}
                      >
      ...
      <section className="py-24 bg-luxury-glow-e border-y border-brand-sage/20 px-6 sm:px-10 lg:px-16 animate-none" id="delivery-formats">
        <div className="max-w-[1280px] mx-auto">
          <div className="text-center max-w-xl mx-auto mb-16 space-y-4">
            ...
            <p className="text-xs sm:text-sm text-brand-charcoal/60 font-sans max-w-md mx-auto leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
            ...
            <div className="bg-white/95 p-8 sm:p-10 rounded-[28px] border border-brand-sage/40 flex flex-col justify-between shadow-xs hover:shadow-md transition-all duration-300 hover:-translate-y-0.5" id="offline-studio-details">
              <div className="space-y-5">
                <div className="h-12 w-12 rounded-xl bg-brand-emerald/10 text-brand-emerald flex items-center justify-center shrink-0">
                  <Coffee className="h-6 w-6 text-brand-emerald" />
                </div>
                ...
                <div className="pt-2 space-y-2.5">
                  <div className="flex items-center gap-2.5 text-xs text-brand-charcoal/75 font-sans">
                    <Check className="h-4 w-4 text-brand-emerald shrink-0" />
            ...
            <div className="bg-white/95 p-8 sm:p-10 rounded-[28px] border border-brand-sage/40 flex flex-col justify-between shadow-xs hover:shadow-md transition-all duration-300 hover:-translate-y-0.5" id="online-stream-details">
              <div className="space-y-5">
                <div className="h-12 w-12 rounded-xl bg-brand-emerald/10 text-brand-emerald flex items-center justify-center shrink-0">
                  <Globe className="h-6 w-6 text-brand-emerald" />
                </div>
